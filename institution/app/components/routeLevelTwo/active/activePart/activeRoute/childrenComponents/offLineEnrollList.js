...
export default class OffLineEnrollList extends React.Component{
    constructor(props){
        super(props)
        this.state = {
            'pageNumber':[],
    ...
    render(){
        return(<div id="offLineEnrollList" style={this.props.hideStyle}>
                    ...
                    this.state.items.map(function(item,index){
                        return (
                            <tr key={index}>
                                ...
                                <td>{item.applicant?item.applicant:'-'}</td>
    ...
    componentWillReceiveProps(nextProps){
        if(nextProps.searchOffLineEnroll!='' || nextProps.status!=0){
            this.getEnrollList(nextProps.status,0,nextProps.searchOffLineEnroll);
        }
    }
    getEnrollList(activityId,page,search){
        ...
        sendAjax({
            type:"POST",
            url:"admin/activity/findSpotRegister",
            data:{
                token:token,
    ...
    changePage(page,e){
        ...
        //获取数据
        this.getEnrollList(this.props.status,page,this.state.searchOffLineEnroll);
        this.setState({
            pageNow:page+1
        })
    }
    jumpToFirst(){
        this.getEnrollList(this.props.status,0,this.state.searchOffLineEnroll);
        $('.pageContainer').find('li').css({'background':'#fff','color':'#000'})
    ...
    jumpToLast(){
        this.getEnrollList(this.props.status,this.state.pageAll-1,this.state.searchOffLineEnroll);
        $('.pageContainer').find('li').css({'background':'#fff','color':'#000'})
        this.setState({
    ...
    jumpToPages(){
        ...
        if(page!=''){
            if(page>0&&page<=this.state.pageAll){
                this.getEnrollList(this.props.status,page-1,this.state.searchOffLineEnroll);
                this.setState({
                    pageNow:page
                })
            }
